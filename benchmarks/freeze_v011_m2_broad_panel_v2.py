#!/usr/bin/env python3
"""Create the prospective 14-CPU v0.11 M2 successor contract."""
...
def freeze(output: Path) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"v0.11 M2 v2 contract is create-only: {output}")
    status = campaign._v1._base.hardened._repository_status(campaign.ROOT, None)
    if status:
        raise RuntimeError("freeze from a clean committed M2 v2 harness checkout")
