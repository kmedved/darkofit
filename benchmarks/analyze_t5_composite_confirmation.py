...
def _atomic_create(path, text, *, _keep_open=False):
    encoded = text.encode()
...
def _atomic_create_pair(first_path, first_text, second_path, second_text):
    ...
    try:
        created.append(
            (
                first_path,
                _atomic_create(first_path, first_text, _keep_open=True),
            )
        )
        created.append(
            (
                second_path,
                _atomic_create(second_path, second_text, _keep_open=True),
            )
        )
    except BaseException:
