"""Run the 14-CPU successor to the frozen v0.11 M2 broad panel."""
...
from benchmarks import run_v011_m2_broad_panel as _v1


ROOT = Path(__file__).resolve().parents[1]
_V1_CONTRACT_ID = _v1.CONTRACT_ID
_V1_PROTOCOL = _v1.frozen_protocol()
CONTRACT_ID = "v011-m2-broad-panel-20260722-v2"
CONTRACT_PATH = ROOT / "benchmarks/v011_m2_broad_panel_contract_v2_20260722.json"
EXPECTED_CHILD_CPUS = 14
DEFAULT_OUTPUT_DIR = Path(".cache/v011-m2-broad-panel-v2-20260722")
WORKER_ENVIRONMENT = {
    **_v1.WORKER_ENVIRONMENT,
...
CAMPAIGN_KIND = "darkofit_v011_m2_broad_panel_v2_20260722"
COMPLETION_KIND = CAMPAIGN_KIND + "_completion"
PAYLOAD_KIND = CAMPAIGN_KIND + "_analysis_payload"

SOURCE_FILES = (
    *_v1.SOURCE_FILES,
    Path("benchmarks/v011_m2_broad_panel_v1_preflight_failure_20260722.md"),
    Path("benchmarks/run_v011_m2_broad_panel_v2.py"),
    Path("benchmarks/analyze_v011_m2_broad_panel_v2.py"),
    Path("benchmarks/freeze_v011_m2_broad_panel_v2.py"),
    Path("benchmarks/v011_m2_broad_panel_protocol_v2_20260722.md"),
    Path("benchmarks/v011_m2_broad_panel_contract_v2_20260722.json"),
)
BOUND_PATHS = {
    "authorization": Path("benchmarks/v011_evidence_phase_instruction_20260721.md"),
    "v1_preflight_record": Path(
        "benchmarks/v011_m2_broad_panel_v1_preflight_failure_20260722.md"
    ),
    "protocol": Path("benchmarks/v011_m2_broad_panel_protocol_v2_20260722.md"),
    "runner": Path("benchmarks/run_v011_m2_broad_panel_v2.py"),
    "analyzer": Path("benchmarks/analyze_v011_m2_broad_panel_v2.py"),
    "freezer": Path("benchmarks/freeze_v011_m2_broad_panel_v2.py"),
    "tests": Path("tests/test_v011_m2_broad_panel_v2.py"),
}
...
def frozen_protocol() -> dict[str, Any]:
    protocol = copy.deepcopy(_V1_PROTOCOL)
    protocol["successor"] = {
        "supersedes_contract_id": _V1_CONTRACT_ID,
        "reason": "v1_preflight_host_exposes_14_not_18_cpus",
        "v1_fit_count": 0,
        "v1_campaign_artifact_count": 0,
        "only_protocol_change": "common_cpu_and_thread_budget_18_to_14",
    }
    ...
    protocol["execution_dispatch"]["worker_environment"] = dict(WORKER_ENVIRONMENT)
    protocol["execution_dispatch"]["resource_basis"] = (
        "equal_live_host_maximum_14_logical_physical_active_cpus"
    )
    protocol["num_cpus"] = EXPECTED_CHILD_CPUS
    return protocol
...
def load_contract(path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("v0.11 M2 v2 contract is invalid")
    for name, relative in BOUND_PATHS.items():
        if payload["bindings"][name] != _bound_record(ROOT / relative):
            raise RuntimeError(f"v0.11 M2 v2 contract binding drifted: {name}")
    freeze_head = str(payload.get("harness_freeze_git_head", ""))
    if len(freeze_head) != 40:
        raise RuntimeError("v0.11 M2 v2 harness freeze commit is invalid")
    ...
    try:
        ...
        raise RuntimeError("current source does not descend from the M2 v2 freeze") from exc
    return payload
...
def validate_execution_source_pin(contract: Mapping[str, Any]) -> str:
    ...
    if revision != [current, freeze_head]:
        raise RuntimeError("M2 v2 source must be the direct contract-commit child")
    changed = _v1._base._run_git(
        ["diff", "--name-only", freeze_head, current], cwd=ROOT
    ).splitlines()
    if changed != [str(CONTRACT_PATH.relative_to(ROOT))]:
        raise RuntimeError("M2 v2 contract commit changed more than its contract")
    if _v1._base._run_git(["rev-parse", "origin/main"], cwd=ROOT) != current:
        raise RuntimeError("M2 v2 execution source is not published at origin/main")
    return current
