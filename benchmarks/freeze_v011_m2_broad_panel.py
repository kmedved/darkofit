#!/usr/bin/env python3
"""Create the prospective v0.11 M2 broad-panel contract."""
...
def freeze(output: Path) -> dict:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"v0.11 M2 contract is create-only: {output}")
    status = campaign._base.hardened._repository_status(campaign.ROOT, None)
    if status:
        raise RuntimeError("freeze from a clean committed M2 harness checkout")
    head = campaign._base._run_git(["rev-parse", "HEAD"], cwd=campaign.ROOT)
    bindings = {
