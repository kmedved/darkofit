...
def assert_worker_environment(threads: int) -> dict[str, Any]:
    expected = {key: str(int(threads)) for key in THREAD_ENV_KEYS}
    expected.update(
        {
            "DARKOFIT_WARMUP": "0",
            "NUMBA_DISABLE_JIT": "0",
    ...
    actual = {name: os.environ.get(name) for name in (*expected, "NUMBA_CACHE_DIR")}
    mismatches = {
        name: {"expected": value, "actual": actual.get(name)}
        for name, value in expected.items()
        if actual.get(name) != value
    }
    if not actual.get("NUMBA_CACHE_DIR"):
        mismatches["NUMBA_CACHE_DIR"] = {
            "expected": "nonempty absolute path",
            "actual": actual.get("NUMBA_CACHE_DIR"),
        }
    if mismatches:
        raise RuntimeError(
...
def calibration_worker(
    spec: Mapping[str, Any], *, source: str, source_root: Path
) -> dict[str, Any]:
    ...
    runtime_before = assert_worker_environment(int(spec["threads"]))
    import numba
...
def _run_worker(
    ...
    *,
    ...
    completed = subprocess.run(
        command,
        cwd=ROOT,
        env=fixed_worker_environment(threads, cache_dir),
        check=False,
        capture_output=True,
        text=True,
    )
        ...
        for spec in campaign.calibration_specs():
            rows.append(
                _run_worker(
                    ...
                    cache_dir=cache_root / spec["coordinate_id"],
...
def run_validation(
    *,
    ...
    threshold = threshold_record.get("selected", {}).get("threshold")
    if (
        threshold_record.get("qualifies") is not True
    ...
    output = _assert_fresh_output(output)
    cache_root = ROOT / ".cache" / campaign.CAMPAIGN_NAME / "validation"
                ...
                for arm in order:
                    rows.append(
                        _run_worker(
                            ...
                            source=source,
                            source_root=source_root,
                            cache_dir=cache_root / f"{spec['cell_id']}-{block}-{arm}",
...
def analyze_raw(
    *,
    ...
    elif phase == "validation" and threshold is not None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    calibration_worker_parser.add_argument("--spec-json", type=_parse_spec, required=True)
    ...
    validation_worker_parser.add_argument("--spec-json", type=_parse_spec, required=True)
    validation_worker_parser.add_argument("--arm", choices=("fused", "auto"), required=True)
    validation_worker_parser.add_argument("--block", type=int, required=True)
    validation_worker_parser.add_argument("--threshold", type=int, required=True)
    ...
    analyze_parser.add_argument("--threshold", type=int)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "calibration-worker":
        result = calibration_worker(
            args.spec_json, source=args.source, source_root=args.source_root
        )
