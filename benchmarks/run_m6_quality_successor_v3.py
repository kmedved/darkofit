#!/usr/bin/env python3
"""Execute one exact M6 quality-successor v3 development comparison."""
...
CONTRACT_PATH = RUNNER_PATH.with_name("m6_quality_successor_v3_contract.md")
RULE_PATH = RUNNER_PATH.with_name("m6_quality_rule_v3.py")
BACKTEST_RUNNER_PATH = RUNNER_PATH.with_name(
    "run_m6_quality_successor_v3_backtest.py"
)
AUDIT_PATH = RUNNER_PATH.with_name("premature_kill_audit_20260722.json")
SUPERSESSION_PATH = RUNNER_PATH.with_name(
    "m6_quality_successor_v2_phase_f_supersession_20260722.md"
)
...
POSITIVE_PATH = RUNNER_PATH.with_name(
    "m3b_ensemble_v3_r3_vs_single_readout_20260721.json"
)
NEGATIVE_PATH = RUNNER_PATH.with_name("native_ordinal_c2_development_result.json")
RETIRED_SELECTOR_PATH = RUNNER_PATH.with_name("fresh_selector_confirmation.json")
BACKTEST_RESULT_PATH = RUNNER_PATH.with_name(
    "m6_quality_successor_v3_backtest_result.json"
)
...
def source_state(repository: Path) -> dict[str, Any]:
    ...
    if root != repository:
        raise RuntimeError(
            f"M6 v3 source must name its Git root: {repository} (root is {root})"
        )
    status = _git(
...
def validate_backtest_binding() -> dict[str, Any]:
    if not BACKTEST_RESULT_PATH.is_file():
        raise RuntimeError("M6 v3 backtest result is not committed")
    harness = source_state(REPO_ROOT)
    if not harness["clean"]:
        raise RuntimeError("M6 v3 execution requires a clean harness")
    if BACKTEST_RESULT_PATH.read_bytes() != _tracked_head_bytes(BACKTEST_RESULT_PATH):
        raise RuntimeError("M6 v3 backtest result differs from HEAD")
    ...
    binding_paths = {
        ...
        "backtest_runner": BACKTEST_RUNNER_PATH,
        "comparison_runner": COMPARISON_PATH,
    ...
    expected_bindings = {
        name: file_sha256(path) for name, path in binding_paths.items()
    }
    actual = {
        name: payload.get("bindings", {}).get(name, {}).get("sha256")
        for name in expected_bindings
    }
    backtest_head = payload.get("harness", {}).get("head")
    if (
        ...
        or payload.get("shipping_or_default_claim_eligible") is not False
        or payload.get("rerun_authorized") is not False
        or payload.get("supersedes_forward_ranking_authority_of")
        != "m6-quality-successor-v2"
        or len(payload.get("replays", ())) != 3
        or not all(replay.get("agreement") is True for replay in payload["replays"])
        or payload.get("harness", {}).get("clean") is not True
        or actual != expected_bindings
        or not isinstance(backtest_head, str)
        or not _is_ancestor(backtest_head, harness["head"])
    ):
        raise RuntimeError("M6 v3 backtest binding is invalid or drifted")
    return {
        "harness": harness,
        "backtest_harness_head": backtest_head,
        "result_sha256": file_sha256(BACKTEST_RESULT_PATH),
    }
...
def _require_external_output(path: Path) -> None:
    ...
    try:
        ...
        return
    raise ValueError("M6 v3 output paths must be outside the harness checkout")
...
def run(args: argparse.Namespace) -> int:
    ...
    for path in (raw_csv, output, manifest_path):
        ...
        if path.exists() or path.is_symlink():
            raise FileExistsError(f"M6 v3 output is create-only: {path}")
    if (
    ...
    if any(not state["clean"] for state in before.values()):
        raise RuntimeError("M6 v3 control and candidate must be clean")
    if before["control_default"]["head"] == before["candidate_default"]["head"]:
        raise RuntimeError("M6 v3 control and candidate commits must differ")
    ...
    if after != before:
        raise RuntimeError("M6 v3 source state changed during execution")
    ...
    manifest = {
        ...
        "bindings": {
            ...
            "backtest_runner_sha256": file_sha256(BACKTEST_RUNNER_PATH),
