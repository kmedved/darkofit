...
| Panel 3 required an explicit spent-data power go/no-go before fresh access. | Closed at the power gate. The repaired H1/H2 chain produced 117/117 spent calibration results and reproduced every substantive ratio, engagement state, and aggregate from the retired invalid-publication attempt. T5 reached `50.00%` simulated pass probability (one-sided Wilson lower bound `48.61%`) and guarded cross-features reached `10.64%` (`9.82%`), against the required `80.00%`; therefore `NO-GO`. See [`panel3_power_design_decision.json`](panel3_power_design_decision.json). |
| The composite and guarded-cross candidates needed separate prospective adjudication. | Both were adjudicated without post-outcome selection by the preregistered simulation. Neither survived, so the decision retains zero candidates and every fresh-access, registry, confirmation, default-promotion, and product-claim authorization remains false. |
| Deterministic Panel 3 primaries, reserves, and execution machinery remained conditional. | Unused by design after the `NO-GO`. No fresh target preflight, registry, confirmation result, or lockbox spool was created; the CTR23 lockbox remains sealed. |
