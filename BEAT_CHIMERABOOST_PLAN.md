...
| R1-4 | **Powered fresh Tier-D panel design** — one shared design effort (≥80% power, Panel 3's lesson) serving the queued T7b automatic-depth policy (`eligible_for_fresh_tier_d_design`); the killed selector is excluded | **Next authorized design slot.** Design only; owner separately authorizes any fresh run. |
