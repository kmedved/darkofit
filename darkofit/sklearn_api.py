...
class DarkoRegressor(RegressorMixin, _RefitParamsMixin, BaseEstimator):
    ...
    def _automatic_linear_selector_fallback_metadata(self, reason):
        if hasattr(self, "ensemble_metadata_"):
            ...
            fit_seed = getattr(
                model,
                "_fit_random_state_seed_",
                None,
            )
            final_requested = bool(
                getattr(model, "linear_leaves", False)
            )
            final_active = bool(
    ...
    def _automatic_linear_selector_ineligible_reason(
        ...
        if callbacks:
            return "callbacks"
        if self.ordered_boosting is True or isinstance(
            self.ordered_boosting, np.bool_
    ...
    def _fit_automatic_linear_selector(
        ...
        *,
        ...
        if fit_seed is None:
            ...
            # Resolve the wrapper's stochastic None policy once, record it,
            # and use the same seed for the final selected fit.
            fit_seed = 0
        if eval_set is None:
            ...
            if group_values is not None:
                ...
                if not group_disjoint:
                    raise RuntimeError(
                        "automatic linear selector split groups are not disjoint"
                    )
            split = {
